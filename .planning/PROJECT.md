...
- [ ] Политика конфиденциальности и iOS Privacy Manifest (`PrivacyInfo.xcprivacy`) подготовлены уже в v1, даже без публичной публикации
- [ ] Прогноз итоговой оценки и график динамики среднего балла как cheap differentiators
...
- **Публичная публикация в App Store / Google Play в v1** — Отложена в v2: сначала стабильная сборка для семейного/классного использования через TestFlight + Google Play Internal track, потом публичная публикация после согласований
- **Формальная регистрация в Роскомнадзоре и письменное согласие школы №28 в v1** — Отложены в v2: v1 рассматривается как личное/семейное использование; legal формализация — milestone после v1
- **Биометрическая разблокировка приложения в v1** — Защита кредов обеспечивается платформенным Keychain/Keystore + системным паролем устройства; биометрия как доп. удобство — v2
...
| v1 как «личное/семейное использование» через TestFlight + Google Play Internal track | Снимает блокирующие legal‑риски (152‑ФЗ, ToS АВЕРС, согласие школы) до момента, когда приложение технически готово; формализация переносится в v2 после реальной валидации | — Pending |
| Public submission в App Store / Google Play отложен в v2 | Submission требует privacy/data‑safety деклараций, app review, маркетинговых материалов — преждевременно для v1, лучше когда приложение стабильно работает в малом круге | — Pending |
| Прогноз итоговой оценки и график динамики среднего балла включены уже в v1 | Cheap differentiators поверх локальных вычислений — главные родительские запросы; нет дополнительной зависимости от API (формула считается на устройстве) | — Pending |
| Биометрическая разблокировка отложена в v2 | KeyChain/Keystore + системный пароль устройства уже обеспечивают защиту кредов; биометрия — UX‑улучшение, не требование безопасности | — Pending |
...
---
*Last updated: 2026-04-27 after requirements scoping (submission и formal legal compliance перенесены в v2; v1 — личное/семейное использование через TestFlight + Internal track)*
